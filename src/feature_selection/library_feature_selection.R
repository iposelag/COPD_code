...
load_gene_list <- function(genes_list) {
  file_path <- paste0("../COPD/input_data/feature_selection/",genes_list, ".txt")
...
obtain_mrmr <- function(expression_data, target_var, threshold_value = NULL, directory_to_save = NULL){
    ...
    if(is.null(threshold_value)){
      all_mrmr_scores_df <- run_mrmr_bootstrapping(expression_data, target_var, n_iterations = 5, directory_to_save = NULL)
...
shuffling_sample_disease_categories <- function(data, target_var, directory_to_save = NULL){
  ...
  if (!is.null(directory_to_save)) {
    ...
    directory <- paste0(directory_to_save,"feature_selection/")
    # Save list of genes: input for models
    write.csv(data, paste0(directory,"shuffled_dataset.csv"), row.names = NULL)
  }
...
run_mrmr <- function(data, target_var, variables, samples) {
  ...
  write.csv(data, temp_file, row.names = NULL)
...
extract_mrmr_features <- function(results) {
  ...
  features_df <- read.table(text = feature_lines, header = NULL, sep = "\t", stringsAsFactors = NULL)
...
run_mrmr_bootstrapping <- function(expression_data, target_var, n_iterations = 1000, directory_to_save = NULL) {
  ...
  for (i in 1:n_iterations) {
    ...
    write.csv(all_mrmr_results_df, output_file, row.names = NULL)
    }
    ...
    all_scores_df <- rbind(all_mrmr_results_df, iteration_score_df)
  }
...
obtain_omnipath_combined <- function(disease_code, directory_to_load, directory_to_save = NULL, operation = "intersection") {
  ...
  data_driven <- obtain_data_driven_genes(directory_to_load)
...
run_feature_selection <- function(procedure, expression_data, target_var, threshold_value =NULL, disease_code = NULL, dea_genes = NULL, directory_to_load = NULL, directory_to_save = NULL) {
  ...
  switch(procedure,
         ...
         "data_driven" = {
           genes_list <- obtain_data_driven_genes(dea_genes, mrmr_genes = NULL, expression_data, target_var, threshold_value, directory_to_save)
         ...
         "omnipath_data_driven" = {
           data_driven <- obtain_data_driven_genes(dea_genes, mrmr_genes = NULL, expression_data, target_var, threshold_value, directory_to_save)
           all_results <- obtain_omnipath_expansion(data_driven,  directory_to_save)
         },
         
         "omnipath_intersection" = {
           genes_list <- obtain_omnipath_combined(disease_code, directory_to_load, directory_to_save, operation = "intersection")
         },
         
         "omnipath_union" = {
           genes_list <- obtain_omnipath_combined(disease_code, directory_to_load, directory_to_save, operation = "union")
         },
